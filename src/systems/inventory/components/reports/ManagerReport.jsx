          ...
          <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
            <thead className="sticky top-0 z-20 bg-white text-slate-600 text-xs font-bold uppercase tracking-wider shadow-sm border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-right text-rose-600">Withdrawal</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {reportsData.length === 0 ? (
                ...
                reportsData.map((row, idx) => {
                  ...
                  const cash = parseFloat(row.cash_amount ?? row.cash ?? 0);
                  const expense = parseFloat(row.expense_amount ?? row.expense ?? 0);
                  const withdrawal = parseFloat(row.withdrawal_amount ?? 0);
                  const balance = parseFloat(row.balance ?? 0);
                  const isWithdrawal = row.entry_type === 'withdrawal';

                  return (
                    <tr key={row.id || idx} className={`hover:bg-slate-50/40 transition-colors ${isWithdrawal ? 'bg-rose-50/40' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-slate-600 font-medium whitespace-nowrap">
                        ...
                        {isWithdrawal && (
                          <span className="ml-2 inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide bg-rose-100 text-rose-600">
                            Withdrawal{row.performed_by ? ` by ${row.performed_by}` : ''}
                          </span>
                        )}
                      </td>
