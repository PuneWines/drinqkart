...
/**
 * Check whether Closing Stock has been submitted for a given date + shop.
 * Used to warn/block a Cash Tally (Sale Amount) submission that would
 * otherwise compute Total Sales Amt as ₹0 (sale_qty is only derived
 * once closing_qty is known for that date).
 */
export async function hasClosingStockForDate(date, shopId) {
  try {
    ...
    console.error('Failed to check closing stock for date:', err.message);
    throw err;
  }
}
 ...
 * Submit Daily Sales Summary Transaction (Mode 3)
 */
export async function submitSaleAmountTransaction(date, gpay, cash, expense, totalClosing, shopId, withdrawal = 0) {
  try {
    // 1. Insert transaction
    const { data: tx, error: txErr } = await supabase
      .from('Inventory_inventory_transactions')
    ...
    (itemsData || []).forEach(item => {
      mrpMap[item.id] = parseFloat(item.mrp) || 20;
    });

    // Calculate total sales from ledger
    let totalSalesAmt = 0;
    salesData.forEach(row => {
      const qty = parseFloat(row.sale_qty) || 0;
      const mrp = mrpMap[row.item_id] !== undefined ? mrpMap[row.item_id] : 20;
...
export async function getDailySalesSummary({ fromDate, toDate, shopId, limit = 500 } = {}) {
  try {
    ...
    return (data || []).map(row => ({
      ...
      withdrawal_amount: parseFloat(row.withdrawal_amount) || 0
    }));
  } catch (err) {
...
export async function recalculateManagerReportBalances(shopName, startDate) {
  ...
  try {
    ...
    for (const report of (reports || [])) {
      ...
      const withdrawal = parseFloat(report.withdrawal_amount) || 0;
