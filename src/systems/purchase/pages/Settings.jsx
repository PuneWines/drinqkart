...
const Settings = ({ readOnly = false }) => {
  ...
  const fetchTransporters = async () => {
    const { data } = await supabase.from("purchase_transporters").select("*").order("created_at", { ascending: false });
    if (data) setTransporters(data);
  };

  const fetchReceivers = async () => {
    const { data } = await supabase.from("purchase_receivers").select("*").order("created_at", { ascending: false });
  ...
  const fetchAvailableParties = async () => {
    try {
      const [res1, res2] = await Promise.all([
        supabase.from("purchase_indent_items").select("party_name").not("party_name", "is", null),
        supabase.from("purchase_approved_indent_items").select("party_name").not("party_name", "is", null)
      ]);
  ...
  const handleTransporterSubmit = async (e) => {
    ...
    if (editingTransporter) {
      res = await supabase.from("purchase_transporters").update(transporterFormData).eq("id", editingTransporter);
    } else {
      res = await supabase.from("purchase_transporters").insert([transporterFormData]);
    }
  ...
  const handleDeleteTransporter = async (id) => {
    if (window.confirm("Delete this transporter?")) {
      await supabase.from("purchase_transporters").delete().eq("id", id);
      fetchTransporters();
    }
  };
  ...
  const handleReceiverSubmit = async (e) => {
    ...
    let res;
    if (editingReceiver) {
      res = await supabase.from("purchase_receivers").update(receiverFormData).eq("id", editingReceiver);
    } else {
      res = await supabase.from("purchase_receivers").insert([receiverFormData]);
    }
  ...
  const handleDeleteReceiver = async (id) => {
    if (window.confirm("Delete this receiver?")) {
      await supabase.from("purchase_receivers").delete().eq("id", id);
      fetchReceivers();
    }
  };
