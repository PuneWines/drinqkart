...
export const fetchPipelineOrders = async ({
  ...
  let query = supabase.from("purchase_purchase_orders").select("*");
  ...
  const [transportersRes, receiversRes] = await Promise.all([
    supabase.from("purchase_transporters").select("name, contact_number"),
    supabase.from("purchase_receivers").select("name, contact_number"),
  ]);
