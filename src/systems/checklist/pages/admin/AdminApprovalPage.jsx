...
} from "../../redux/api/delegationApi";
import {
  fetchPendingMaintenanceApprovals,
  approveMaintenanceTask,
  rejectMaintenanceTask,
  fetchApprovedMaintenance,
...
} from "../../redux/api/repairApi";
import {
  fetchPendingEAApprovals,
  approveEATaskV2,
  rejectEATask,
  fetchApprovedEA,
} from "../../redux/api/eaApi";
import {
...
import {
  fetchPendingWorkApprovalsApi,
  fetchWorkTaskHistoryApi,
  approveWorkTaskApi,
  rejectWorkTaskApi,
} from "../../redux/api/workRecordsApi";
import {
  CheckCircle2,
  Search,
  Play,
...
export default function AdminApprovalPage() {
  ...
  const handleExtensionRemark = async (task) => {
    ...
    try {
      ...
      showToast(
        "Failed to send remark: " + (error.message || "Unknown error"),
        "error",
      );
    } finally {
    ...
    try {
      if (viewMode === "pending") {
        if (activeTab === "delegation") data = await fetchPendingApprovals();
          ...
          data = await fetchPendingWorkApprovalsApi(currentUserRole);
      } else {
        // History Mode
        if (activeTab === "delegation") data = await fetchDelegationHistory();
        else if (activeTab === "maintenance")
          data = await fetchApprovedMaintenance();
  ...
  const handleBulkApprove = async () => {
    ...
    if (failCount === 0) {
      ...
      showToast(
        `Approved ${successCount} tasks, ${failCount} failed.`,
        "warning",
      );
    }
  };
  ...
  const handleBulkRemark = async () => {
    ...
    const tasksToRemark = pendingTasks.filter(
      (t) => selectedTaskIds.includes(t.id) && t.status === "extend",
    );
    let count = 0;
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      showToast(
        "Failed to reject task: " + (error.message || "Unknown error"),
        "error",
      );
    } finally {
      setProcessingId(null);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  ...
                  paginatedTasks.map((task) => (
                    <tr
                      key={task.id}
                      className={`hover:bg-gray-50 transition-colors ${selectedTaskIds.includes(task.id) ? "bg-purple-50/50" : ""}`}
                    >
                      {viewMode === "pending" && (
                              ...
                              <div className="flex flex-col">
                                ...
                                {task.manager_approval_date && (
                                  <span className="text-[9px] text-indigo-400 font-medium">
                                    {formatDate(task.manager_approval_date)}
                                  </span>
                                )}
                              </div>
                            </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        {viewMode === "pending" ? (
                          ...
                          <span
                            className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-red-100 text-red-800"
                            title={task.rejection_reason || task.reason}
                          >
                            Rejected
                          </span>
                        ) : task.status === "extend" ? (
          ...
          <div className="md:hidden divide-y divide-gray-100 pb-24">
            {loading ? (
              ...
              paginatedTasks.map((task) => (
                <div
                  key={`card-${task.id}`}
                  className={`p-4 space-y-4 hover:bg-blue-50/30 transition-colors ${selectedTaskIds.includes(task.id) ? "bg-purple-50/80 border-l-4 border-l-purple-500" : ""}`}
                >
                        ...
                        <div className="space-y-1 mt-1">
                                ...
                                <>
                                  <p className="text-[10px] text-indigo-500 font-bold uppercase tracking-wider flex items-center gap-1 mt-0.5">
                                    <CheckCircle2
                                      ...
                                      className="shrink-0"
                                    />{" "}
                                    Mgr. Approved By: {task.manager_approved_by}
                                  </p>
                                  <p className="text-[10px] text-indigo-500 font-bold uppercase tracking-wider flex items-center gap-1 mt-0.5">
                          ...
                          {task.status === "extend" && (
                            <p className="text-[10px] text-amber-600 font-black uppercase tracking-wider flex items-center gap-1 mt-0.5">
                              <Clock size={10} /> Extended To:{" "}
                              {formatDate(task.next_extend_date)}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {viewMode === "pending" ? (
                      ...
                      <div className="text-center space-y-1">
                        ...
                        !["approved", "rejected"].includes(
                          ...
                          <span className="block w-full py-1.5 bg-orange-50 text-orange-700 text-[10px] font-black uppercase tracking-widest rounded-lg">
                            Pending Approval
                          </span>
                        ) : task.status === "MANAGER_APPROVED" ? (
                          <span className="block w-full py-1.5 bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-widest rounded-lg">
                            Manager Approved
                          </span>
                        ) : (
                          <div className="flex flex-col gap-1 items-center">
                            <span className="block w-full py-1.5 bg-green-50 text-green-700 text-[10px] font-black uppercase tracking-widest rounded-lg">
                              Approved ✅
                            </span>
                            {activeTab === "work" &&
                              task.manager_approved_by && (
                                <span className="block text-[8px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded uppercase tracking-wider">
                                  Manager Approved
                                </span>
                              )}
                          </div>
          ...
          <div className="md:hidden fixed bottom-6 left-4 right-4 z-40 animate-in slide-in-from-bottom-8 duration-500">
            <div className="bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] border border-purple-100 p-2 overflow-hidden">
              <div className="flex items-center justify-between gap-2">
                <div className="pl-4">
                  <p className="text-[10px] font-black text-purple-600 uppercase tracking-[0.2em] mb-0.5">
                    Admin Action
                  </p>
                  <p className="text-xs font-bold text-gray-500">
                    {selectedTaskIds.length} items
                  </p>
                </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-gray-400 italic px-1">
                      * User will be notified via WhatsApp including this
                      reason.
                    </p>
                  </div>
