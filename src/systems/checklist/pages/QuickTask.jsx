          ...
          <>
            {activeTab === 'checklist' ? (
              <div className="mt-4 rounded-lg border border-purple-200 shadow-md bg-white overflow-hidden">
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-purple-100 p-2 flex justify-between items-center">
                  <div>
                ...
                <div
                  ...
                  className="overflow-auto max-h-[calc(100vh-280px)] custom-scrollbar"
                >
                  {/* Desktop View */}
                  <table className="hidden md:table min-w-full divide-y divide-gray-200 relative">
                    <thead className="bg-gray-50 sticky top-0 z-20 shadow-xs">
                      <tr>
                        ...
                        {[
                          ...
                          <th
                            key={column.label}
                            className={`px-4 py-2.5 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider ${column.bg || ''} ${column.minWidth || ''} ${column.key && column.key !== 'actions' ? 'cursor-pointer hover:bg-gray-100' : ''}`}
                      ...
                      {filteredChecklistTasks.length > 0 ? (
                        filteredChecklistTasks.map((task, index) => (
                          <tr key={index} className="hover:bg-gray-50">
                            ...
                            {/* Task Description */}
                            <td className="px-4 py-2.5 text-xs text-gray-500 min-w-[300px] max-w-[400px]">
                            ...
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500">
                              {task.task_level ? (
                                <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold uppercase tracking-tighter">
                                  {task.task_level}
                                </span>
                              ) : "—"}
                            </td>
                            ...
                            {/* Task Start Date */}
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500 bg-yellow-50">
                              {formatTimestampToDDMMYYYY(task.task_start_date)}
                            </td>

                            {/* Frequency */}
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] ${task.frequency?.toLowerCase() === 'daily' ? 'bg-blue-100 text-blue-800' :
                            ...
                            {/* Duration */}
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500 bg-blue-50">
                              {task.duration ? (
                    ...
                    <thead className="bg-gray-50 sticky top-0 z-20 shadow-xs">
                      <tr>
                        ...
                        {[
                          ...
                          <th
                            key={column.label}
                            className={`px-4 py-2.5 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider ${column.bg || ''} ${column.minWidth || ''}`}
                          >
                            {column.label}
                          </th>
                        ))}
                      ...
                      {filteredMaintenance.length > 0 ? (
                        filteredMaintenance.map((task, index) => (
                          <tr key={index} className={`hover:bg-gray-50 ${selectedTasks.find(t => t.id === task.id) ? "bg-purple-50" : ""}`}>
                            ...
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500">{task.id}</td>
                            <td className="px-4 py-2.5 text-xs text-gray-500 min-w-[200px] max-w-[400px]">
                              <RenderDescription
                            ...
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] ${task.freq?.toLowerCase() === 'daily' ? 'bg-blue-100 text-blue-800' :
                            ...
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] ${task.status === 'Done' ? 'bg-green-100 text-green-800' :
                      ...
                      {filteredWorkTasks.length > 0 ? (
                        filteredWorkTasks.map((task, index) => (
                          <tr key={index} className={`hover:bg-gray-50 ${selectedTasks.find(t => t.id === task.id) ? "bg-purple-50" : ""}`}>
                            ...
                            <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-500">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] ${task.status === 'GENERATED' ? 'bg-indigo-100 text-indigo-800' :
